...
# States for Edit Conversation (if needed, but for now we handle via individual callbacks)
WAIT_EDIT_VALUE = 70
...
async def admin_free_manage_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"⚙️ **مدیریت کانفیگ: {c.title}**\n\n"
    text += f"🌍 لوکیشن: {c.country}\n"
    text += f"📝 توضیحات: {c.description}\n"
    exp_str = c.expire_date.strftime("%Y-%m-%d %H:%M") if c.expire_date else "دائمی"
    text += f"⏳ زمان انقضا: {exp_str}\n"
    
    keys = [
        [InlineKeyboardButton("✏️ ویرایش نام", callback_data=f"adm_free_ed_title_{fid}"), InlineKeyboardButton("🌍 ویرایش لوکیشن", callback_data=f"adm_free_ed_country_{fid}")],
        [InlineKeyboardButton("📝 ویرایش توضیحات", callback_data=f"adm_free_ed_desc_{fid}"), InlineKeyboardButton("🔗 ویرایش لینک", callback_data=f"adm_free_ed_data_{fid}")],
...
async def admin_free_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if c:
            ...
            await query.edit_message_text(f"✅ کانفیگ '{c.title}' با موفقیت حذف شد.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت به لیست", callback_data="admin_free_configs")]]))
        else:
...
async def save_free_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("مدت زمان اعتبار به روز (عدد وارد کنید، مثلاً 3. برای دائمی عدد 0 را بفرستید):", reply_markup=InlineKeyboardMarkup(CANCEL_BTN))
...
async def start_edit_free_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"مقدار جدید را برای **{field_fa}** وارد کنید:", reply_markup=InlineKeyboardMarkup(CANCEL_BTN), parse_mode="Markdown")
...
async def save_edit_free_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        c = (await session.execute(select(FreeConfig).where(FreeConfig.id == fid))).scalars().first()
        if not c:
            await update.message.reply_text("خطا: کانفیگ یافت نشد.")
            return ConversationHandler.END
...
def get_admin_free_routers():
    return [
        ...
        MessageHandler(filters.Regex(r'^/del_free_\d+'), del_free_command) # Keep legacy for now
    ]

async def del_free_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # This was the old way, but we now have better UI. Keeping it as a backup or for quick delete.
    ...
    async with AsyncSessionLocal() as session:
        ...
        if c:
            await session.delete(c)
            await session.commit()
            await update.message.reply_text(f"🗑 کانفیگ #{fid} حذف شد.")
