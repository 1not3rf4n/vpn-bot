    ...
    async with AsyncSessionLocal() as session:
        ...
        if product.product_type == 'V2RAY':
            ...
            if panel_db:
                ...
                if uuid_res:
                    ...
                    # Fetch subscription ID that was generated by the panel
                    sub_id = await client.get_client_subscription_id(inbound_id, email)
                    if not sub_id:
                        sub_id = email  # Fallback to email if sub_id not found
                        logger.warning(f"Could not get subscription ID for {email}, using email as fallback")
                    logger.info(f"Client subscription ID: {sub_id}")
                    ...
                    if direct_link:
                        ...
                        logger.info(f"V2RAY provisioned OK: {remark}, inbound={inbound_id}, api={client.api_mode}, sub_id={sub_id}")
                    else:
        ...
        if config_link:
            svc.config_link = build_service_config_link(
                ...
                sub_id=sub_id if product.product_type == "V2RAY" else "",
            )
        else:
